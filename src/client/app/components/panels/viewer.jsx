...
import Plot from 'client/app/components/panels/plot.jsx';
import TFTable from 'client/app/components/panels/tfTable.jsx';
import TFSummaryTable from 'client/app/components/panels/tfSummaryTable.jsx';
import TFPlot from 'client/app/components/panels/tfPlot.jsx';
...
class Viewer extends React.Component {
  ...
  updateDimensions() {
    if (this.refs && this.refs.viewerContainer) {
      ...
      this.setState({
        clientWidth: parseFloat(clientWidth - (2 * this.state.clientMargin) - (2 * this.state.clientPadding)),
        clientHeight: parseFloat(clientHeight - (2 * this.state.clientMargin) - (2 * this.state.clientPadding) - this.state.navbarHeight)
      });
    }
  }
    ...
    if (this.props.settingsChangeStart) {
      ...
      this.props.updateSettingsChange(false, function() {
        self.setState({
          ...
          console.log("viewer - componentDidUpdate() - issuing POST request...", self.props.settings.viewMode);
  ...
  updateAssociationData() {
    ...
    var single_sample_query = "https://forge2-tf.altiusinstitute.org/assets/services/query.py";
    var aggregate_sample_query = "https://forge2-tf.altiusinstitute.org/assets/services/query_aggregate.py";
    var sample_label = settings.sample;
    var se = sample_label.split("-");
    var sample = se[0];
    var ds = se[1];
    var sample_query = (ds == "aggregate") ? aggregate_sample_query : single_sample_query;
    var self = this;
    axios.post(sample_query, { settings })
      .then(
        res => {
  ...
  updateSummaryData() {
    ...
    var tf_summary_query = "https://forge2-tf.altiusinstitute.org/assets/services/query_tf_summary.py";
    var self = this;
    axios.post(tf_summary_query, { tf_summary })
      .then(
        res => {
  ...
  updateSelectedTF(rowInfo) {
    ...
    this.setState({
      ...
      var tf_aggregate_summary_query = "https://forge2-tf.altiusinstitute.org/assets/services/query_tf_aggregate_summary.py";
      var self = this;
      axios.post(tf_aggregate_summary_query, { tf_aggregate_summary })
        .then(
          agg_result => {
            console.log("viewer - updateSelectedTF() - agg_result");
            console.log(agg_result.data);
            var tf_probe_overlap_query = "https://forge2-tf.altiusinstitute.org/assets/services/query_tf_probe_overlap_summary.py";
            ...
            axios.post(tf_probe_overlap_query, { tf_probe_overlap_summary })
              .then(
                overlap_result => {
                  ...
                  overlap_result.data['overlaps']['probes'] = overlap_result.data['overlaps']['probes']['probes'];
                  var tf_query_probe_matches = [];
                  self.props.settings.probes.forEach(function(targetProbe) {
                    overlap_result.data['overlaps']['probes'].forEach(function(queryProbes, queryPositionIndex) {
                      if (!queryProbes)
                        return;
                      queryProbes.forEach(function(queryProbe) {
                        if (targetProbe === queryProbe) {
                          console.log("match of", targetProbe, "at position", queryPositionIndex);
    ...
    if (this.props.settings && (this.props.settings.probesCount >= 1)) {
      return (
        <div className="viewer-gallery-header" id="viewer-gallery-header">
          <div>
            <button type="button" disabled={this.props.settings.probesCount == 1} value="viewer-go-previous" className="react-bootstrap-button-custom-style btn btn-default" name="viewer-go-previous" onClick={this.handleInputChange}><FaAngleLeft /></button>
          </div>
          <div className="viewer-gallery-header-title">SNP TF and Footprint Associations</div>
          <div>
            <button type="button" disabled={this.props.settings.probesCount == 1} value="viewer-go-next" className="react-bootstrap-button-custom-style btn btn-default" name="viewer-go-next" onClick={this.handleInputChange}><FaAngleRight /></button>
          </div>
        </div>
      );
    }
      ...
      <div>
        ...
        <div className="viewer-tf-table-container">
          <div className="viewer-paragraph" id="viewer-hint">
            <em>Hint</em>: Move your mouse pointer over a row to highlight a transcription factor binding site. Click once to lock the highlight, and click again to unlock.
          </div>
          <div className="viewer-tf-table-container">
            <TFTable 
              id="tf-table"
    ...
    if (this.props.settings.viewMode == "Associations") {
      return (
        <div>
          ...
          { this.state.associationDataAvailable && this.renderAssociationBody() || this.renderEmptyBody() }
        </div>
      );
    }
    else if (this.props.settings.viewMode == "Summary") {
      return (
        <div>
          ...
          { this.state.summaryDataAvailable && this.renderSummaryBody() || this.renderEmptyBody() }
        </div>
      );
    }
    else {
