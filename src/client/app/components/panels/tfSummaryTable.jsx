...
import * as AppConst from '../../appConstants.js';
...
class TFSummaryTable extends React.Component {
  ...
  constructor(props) {
    ...
    this.convertTFSummaryToSummaryObject = this.convertTFSummaryToSummaryObject.bind(this);
    this.convertTFSummaryToReactTableDataObj = this.convertTFSummaryToReactTableDataObj.bind(this);
  ...
  updateDimensions() {
    if (this.refs && this.refs.tfTableContainer) {
      ...
      if ((clientHeight != this.state.clientHeight) || (clientWidth != this.state.clientWidth)) {
        this.setState({
  ...
  handleInputChange(event, customName, mouseoutFlag) {
    ...
    var value = (mouseoutFlag) ? null : (target.type === 'checkbox') ? target.checked : target.value;
  ...
  exportGraph() {
    function convertToPDF(fName, summary) {
      ...
      var tf_summary_graph_query = "https://forge2-tf.altiusinstitute.org/assets/services/query_tf_summary_graph.py";
      axios({
          method: 'post',
        ...
        .then(
          res => {
            ...
            var blob = new Blob([new Uint8Array(res.data)], { type: 'application/pdf;' });
...
//    console.log("tfTable - componentWillUpdate()", this.timeStampInMs);
  ...
  componentDidUpdate() {
    ...
    setTimeout(() => {
      ...
      var newMaxHeight = parseFloat($("#viewer").height()) - parseFloat($("#plot-container").height()) - 20.0 - 10.0 - parseFloat($("#viewer-summary-header").height()) - parseFloat($("#tf-table-header").height()) - parseFloat($("#tf-table-header").css("margin-bottom")) - parseFloat($("#tf-table-footer").height()) - parseFloat($("#tf-table-footer").css("margin-top"));
    ...
    for (let dbIdx = 0; dbIdx < dbsLength; dbIdx++) {
      ...
      if (tfPvals) {
        tfPvals.forEach(function(element) {
          ...
          let modifiedDb = (db == 'jaspar') ? db + '/JASPAR.vertebrates.meme.pngs' : db;
  ...
  render() {
    const reactTableColumns = [
      { 
        ...
        Cell: row => (
          <div style={{ height:'15px', textAlign:'left', fontSize:'0.9em' }} >{row.value}</div>
        )
      },
      {
        Header: 'Database', 
      ...
      },
      {
        ...
        height: 100,
        Cell: row => (
          <div style={{ height:'auto', width:'auto', maxWidth:'100%', textAlign:'left', fontSize:'0.9em' }} ><img style={{ width:'100%', height:'auto', textAlign:'left' }} src={`assets/services/motif-logos/logos/${row.value.database}/${row.value.name}.png`} onError={this.imgError}/></div>
        )
      },    
    ];
    const summary = this.props.data.summary;
    const probeTFSummary = summary;
    let reactTableData = this.convertTFSummaryToReactTableDataObj(probeTFSummary);
    ...
    var exportTableButton = <Button name="exportTable" key="exportTable" value="exportTable" bsSize="xsmall" onClick={this.handleInputChange} className="react-bootstrap-button-custom-style"><FaExternalLink /> Export CSV</Button>
    
    var exportGraphButton = <Button name="exportGraph" key="exportGraph" value="exportGraph" bsSize="xsmall" onClick={this.handleInputChange} className="react-bootstrap-button-custom-style"><FaExternalLink /> Export PDF</Button>
    
    return (
      <div>
        <div className="tf-table-header" ref="tfTableHeader" id="tf-table-header">
            <p>Hypergeometric <em>p</em>-values are generated from counts of overlaps between a transcription factor and all SNPs of interest, and counts of overlaps between a transcription factor and all SNPs in the selected variant set.</p>
            <p>Multiple-testing correction of <strong>{totalNumberOfTests}</strong> <em>p</em>-value tests (<strong>{totalNumberOfTestsWithOneOrMoreOverlap}</strong> test results displayed, where there is one or more overlaps between SNP and putative TF binding site) is performed with the Benjamini-Yekutieli method (FDR {fdrPct}%) to yield <em>q</em>-values.</p>
        </div>  
        <div className="tf-table-container" ref="tfTableContainer" id={this.props.id}>
          <ReactTable 
            ...
            getTrProps={(state, rowInfo) => {
              ...
              if (rowInfo && (this.state.initialTableRow !== null) && (rowInfo.viewIndex == this.state.initialTableRow) && !this.state.tableInitialized) {
                setTimeout(function() {
              ...
              return {
                onClick: (e) => {
                  this.setState({
                    selectedRow: (rowInfo.index == this.state.selectedRow) ? null : rowInfo.index,
                  }, function() {
                    this.props.updateSelectedTF((this.state.selectedRow !== null) ? rowInfo : null);
                  })
                },
                onMouseOver: (e) => {
                  if (this.state.selectedRow !== null) return;
                  this.setState({
                    mouseoverRow: (rowInfo.index == this.state.mouseoverRow) ? null : rowInfo.index,
                  }, function() {
                    this.props.updateMouseoverTF((this.state.mouseoverRow !== null) ? rowInfo : null);
                  })
                },
                onMouseOut: (e) => {
                ...
                style: {
                  background: (rowInfo.index === this.state.selectedRow) || (rowInfo.index === this.state.mouseoverRow) ? AppConst.settings.style.color.sequenceHighlightedBackground : 'white',
                  color: (rowInfo.index === this.state.selectedRow) || (rowInfo.index === this.state.mouseoverRow) ? 'black' : 'black',
                  fontWeight: (rowInfo.index === this.state.selectedRow) || (rowInfo.index === this.state.mouseoverRow) ? 'bold' : 'normal'
                }
              }
            }}
          />
        </div>
        <div className="tf-table-footer" ref="tfTableFooter" id="tf-table-footer">
